...
from modules import MermaidProcessor, SlideContentGenerator, SlideStructureGenerator


# %% Configuration
class DeckConfig:
    """Configuration class for presentation generation with user inputs."""
    ...
    def __init__(
        ...
        Args:
            ...
            user_prompt: User's presentation request (optional if input_file provided)
        ...
        Raises:
            ValueError: If both input_file and user_prompt are not provided.
            ValueError: If user_prompt is not provided when input_file is None.
        """
        # Validation logic
        ...
        if not input_file and not user_prompt:
            raise ValueError("Either input_file or user_prompt must be provided")
    ...
    def _setup_output_paths(self, output_filename: str, output_dir: str):
        ...
        self.deck_name = clean_name

    def get_summary(self) -> str:
        """Get a summary of the configuration."""
        summary = f"""
=== Presentation Configuration ===
Presentation Name: {self.deck_name}
Input File: {self.input_file or 'None (prompt-only generation)'}
...
# %% Utility Functions
def convert_to_ppt(input_file, output_filename):
    """Convert markdown to ppt using pandoc"""
    try:
        subprocess.run(["pandoc", "-o", output_filename, input_file], check=True)
        print(f"Successfully converted '{input_file}' to '{output_filename}'.")
...
def setup_openai_client(api_key: Optional[str] = None, api_key_var: Optional[str] = None, api_url: Optional[str] = None):
    """Setup and return OpenAI client."""
    httpx_client = httpx.Client(verify=False)

    if not api_key_var and not api_key:
        raise ValueError("Either api_key or api_key_var must be provided")

    if api_key:
        openai_api_key = api_key
    elif api_key_var:
        openai_api_key = os.getenv(api_key_var)

    open_api_url = api_url or "https://api.openai.com/v1"

    return OpenAI(api_key=openai_api_key, base_url=open_api_url, http_client=httpx_client)


# %% Main Deck Generation Pipeline
def generate_deck(config: DeckConfig):
    """
    ...
    slide_structure, structure_usage = structure_gen.generate_structure(
        user_prompt=config.user_prompt, markdown_path=config.input_file, num_slides=config.num_slides
    )

    # Step 2: Generate slide content
    print("\n2. Generating slide content...")
    slide_content, content_usage = content_gen.generate_content(
        slide_structure=slide_structure, markdown_path=config.input_file
    )
...
#         input_file="./test_files/sample_reference_02.md",
...
#     )
#     generate_deck(config3)

if __name__ == "__main__":
    config = DeckConfig(
        output_filename="test_presentation",
        input_file="./test_files/sample_reference_01.md",
        user_prompt="Create a presentation to summarize the key utilities described.",
        num_slides=10,
        output_dir="./test_files/outputs",
    )
    generate_deck(config)
